...
router.post("/notify", (req, res) => {
    ...
    db.query(
        `INSERT INTO leads (email, name, company, domain, source, tier_interest)
         VALUES (?, ?, ?, ?, 'notify_me', ?)
         ON DUPLICATE KEY UPDATE
           tier_interest = VALUES(tier_interest),
           updated_at = CURRENT_TIMESTAMP`,
        [email.toLowerCase(), clean(req.body?.name, 120), clean(req.body?.company, 160),
         clean(req.body?.domain, 253), clean(req.body?.tier, 40)],
        (err) => {
            if (err) {
